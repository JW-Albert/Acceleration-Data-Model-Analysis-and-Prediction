...
def train_and_evaluate_lr(norm_all_features, selected_features, output_dir=None):
    ...
    # 訓練模型
    lr.fit(norm_all_features.iloc[:, selected_features], norm_all_features.index)
    
    # 預測
    y_pred = lr.predict(norm_all_features.iloc[:, selected_features])
    health_index = lr.predict_proba(norm_all_features.iloc[:, selected_features])[:, 1]
    
    # 評估
    accuracy = accuracy_score(norm_all_features.index, y_pred)
    cv_scores = cross_val_score(lr, norm_all_features.iloc[:, selected_features], 
